...
class EmpresaModel extends Model{
    ...
    public function getMedicoID(){
        ...
        $res=  self::readUDF($query);

        
        $len = sizeof($res);
        if($len>0){
            $ultimoMedico = $res[0]['UltimoMedico']; 
            $ultimoMedico++;
        }
        // $invoiceNumber =  str_pad($invoiceNumber, 7, '0', STR_PAD_LEFT); 
        $query="UPDATE empresa SET UltimoMedico ='$ultimoMedico' where id_centro='1' ";
        self::updateUDF($query) ;
        return  $ultimoMedico;
    }
    ...
    public function getLastNumber($resource,$id_empresa,$padding){
        ...
        "medico": "UltimoMedico",
        "return" : "UltimoCredito"
        }';

        $last_r = json_decode( $a_resorce);

        $recurso= $last_r->$resource;

        $ultimo=1;
        $query="SELECT * from ".$this->db_table." where id_centro='$id_empresa' ";
        $res=  self::readUDF($query);

        
        $len = sizeof($res);
        if($len>0){
            $ultimo = $res[0][$recurso]; 
            $ultimo++;
        }
        $ultimo=str_pad($ultimo, $padding, '0', STR_PAD_LEFT); 
        // $invoiceNumber =  str_pad($invoiceNumber, 7, '0', STR_PAD_LEFT); 
        $this->query = "UPDATE ".$this->db_table." SET ".$recurso." ='$ultimo' where id_centro='$id_empresa' ";      
        $this->set_query();
        return  $ultimo;
    }
