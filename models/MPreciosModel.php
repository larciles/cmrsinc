...
class MPreciosModel extends Model{
    
    public function __construct()
    {
        $this->db_table='MPrecios';
    }
    ...
    public function getPrice($coditm,$codpre) {
        $this->query="SELECT * FROM ".$this->db_table." Where codtipre ='$codpre' and coditems='$coditm' ";
        $this->get_query();
        $num_rows = count($this->rows);
        return $this->rows;//$data;
    }
          ...
          if ($num_rows > 0) {
              ...
              if (isset($_GET["pagina"]))
                ...
                if ($query=='') {
                    ...
                    $consulta = $query." ORDER BY name LIMIT ".$inicio."," . $TAMANO_PAGINA;
                }
