...
require_once '../clases/paginator.class.php';
  ...
  $dbTable="majustes";
  $field="CONVERT(VARCHAR(10),fechajus,110) fechafac, numfactu, observacion nombres  , codajus, usuario ";
  $grupo =  $_POST['grupo'];
  ...
  if (  (isset($_POST['search']) && $_POST['search']!="")  && (isset($_POST['fecha']) && $_POST['fecha']!="")   ) {
     ...
     $filter=$_POST['search'];
     $fecha=$_POST['fecha'];
     $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where fechajus ='$fecha' and CONCAT( numfactu,observacion,codajus) like '%$filter%' " ;
     $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  fechajus ='$fecha' and   CONCAT( numfactu,observacion,codajus) like '%$filter%'  ";
     ...
     if (is_numeric($filter) && $len>=6 ) {
        ...
        $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where CONCAT(numfactu,observacion,codajus) like '%$filter%' " ;
        $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  CONCAT(factcomp,observacion,codajus) like '%$filter%' "; 
     }
      
 }else if(isset($_POST['fecha']) && $_POST['fecha']!=""){

     $fecha=$_POST['fecha'];
     $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where fechajus ='$fecha'" ;
     $query    = "SELECT ".$field." FROM ".$dbTable." Where fechajus ='$fecha'";
 ...
 $links      = ( isset( $_POST['links'] ) ) ? $_POST['links'] : 7;
   ...
   if ($filter!=="") {
    ...
    $query  .=" ORDER BY fechajus DESC";    //MUY IMPORTANTE SIN ESTO NO FUNCIONA LA PAGINACION EN MSSQL
