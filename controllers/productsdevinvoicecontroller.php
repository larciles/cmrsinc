...
$dbTable="VIEW_mnotacre";
$field="CONVERT(VARCHAR(10),fechanot,110) fechafac,numnotcre numfactu,nombres,TotImpuesto,Medico,Status ,statnc statfact,subtotal,descuento,total,monto_flete,id,Status,usuario";

  if (  (isset($_POST['search']) && $_POST['search']!="")  && (isset($_POST['fecha']) && $_POST['fecha']!="")   ) {
         
     $filter=$_POST['search'];
     $fecha=$_POST['fecha'];
     $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where fechanot ='$fecha'  and CONCAT( numnotcre,nombres,Medico,Status,total) like '%$filter%' " ;
     $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  fechanot ='$fecha'  and   CONCAT( numnotcre,nombres,Medico,Status,total) like '%$filter%'  ";
     ...
     if (is_numeric($filter) && $len>=6 ) {
        ...
        $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where CONCAT( numnotcre,nombres,Medico,Status,total) like '%$filter%' " ;
        $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  CONCAT( numnotcre,nombres,Medico,Status,total) like '%$filter%'  "; 
     }
