 ...
 // $fech a=date("Y-m-d")-1;	
  ...
  if (isset($_GET['valueToSearch']) && $_GET['valueToSearch']!="") {
	...
	for ($i=$len; $i >=0 ; $i--) { 
	 ...
	 $qconfirmados = "SELECT COUNT(*) num_rows FROM VIEW_mconsultas_02  where $fielSet='$fecha' and activa='1' and  CONCAT( [Historia],[Cedula],[codclien],[nombres],[telfhabit],descons,Medico) like '%$filter%' and CONCAT( [Historia],[Cedula],[codclien],[nombres],[telfhabit],descons,Medico) like '%$invertFilter%'  and confirmado is not null" ;
	 $qasistidos = "SELECT COUNT(*) num_rows FROM VIEW_mconsultas_02  where $fielSet='$fecha' and activa='1' and  CONCAT( [Historia],[Cedula],[codclien],[nombres],[telfhabit],descons,Medico) like '%$filter%' and CONCAT( [Historia],[Cedula],[codclien],[nombres],[telfhabit],descons,Medico) like '%$invertFilter%'  and ASISTIDOS ='Asistido' ";
     $qnuevos = "SELECT COUNT(*) num_rows FROM VIEW_mconsultas_02  where $fielSet='$fecha' and activa='1' and  CONCAT( [Historia],[Cedula],[codclien],[nombres],[telfhabit],descons,Medico) like '%$filter%' and CONCAT( [Historia],[Cedula],[codclien],[nombres],[telfhabit],descons,Medico) like '%$invertFilter%' and primera_control='1' and ASISTIDOS = 'Asistido' ";
