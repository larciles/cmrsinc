...
function mostrarPO(){
  ...
  if(isset($_POST["meses"])){
	...
	//Orden de compra
	$query="Select * from purchaseOM WHERE status = '1' and type='pr' order by fecha";
	$npo=0;
	$lista;
    ...
    for ($j = 0; $j < $npo; $j++) 
    {
          ...
          if( $in!=""){
		    ...
		    if($chk!=null){
		    	...
		    	$query="SELECT coditems, SUM(cantidad) AS venta From VIEW_PO Where (fechafac >= DateAdd(Month, -$meses , GETDATE())) GROUP BY coditems order by coditems";	
		    }			
    ...
    if ($typepedido=="true") {
    	$query="Select CODITEMS,DESITEMS, Existencia from MInventario WHERE prod_serv in( 'P','h')  and Activo in('1','3') or ( Prod_serv='M' and Inventariable=1 )  order by po_order desc ,desitems";
    } else {
    	$query="Select CODITEMS,DESITEMS, Existencia from MInventario WHERE (prod_serv = 'I')  and Activo=1 order by desitems";
    }
    
	//$query="Select CODITEMS,DESITEMS, Existencia from MInventario WHERE (prod_serv = 'P' or prod_serv = 'I')  and Activo=1 order by desitems";
			...
			$tabla .="<tbody>";
				...
				for ($i=0; $i < $nin; $i++) { 
					...
					if($npo>0)
					{
                     ...
                     $tabla .="<td  id='av-$cod' field='average-$cod'>". $ave."</td>"; #Promedio
					...
					//columna Pedido
					 ...
					 $tabla .="<td id='newpedido-$cod' field='colnuevopedido-$cod'>". $ave*$colPedido."</td>"; #Pedido?
					 //Columna pedido Redondeado
					 $colPedRounded= $ave*$colPedido;
...
function convertPedToPO($pon){
	$dbmsql = mssqlConn::getConnection();
	$fecha=date("Y-m-d");	
	$query="INSERT INTO purchaseOM (pon,fecha,Status) VALUES('$pon','$fecha',1)";
}
...
function selectDesincorporarPO(){	  
    $dbmsql = mssqlConn::getConnection();
	$query="Select * from purchaseOM  WHERE status = '4'  and type='pr' order by fecha";	        
	$npo=0;
	$lista;
	$result=$dbmsql->query($query);

	$select ="<select id='desincor' name='desincor' class='form-control' >";
	    $select .="<option value=''>- - -</option>";
		foreach($result as $row)
		{
			$select .="<option value='".$row["pon"]."'>".$row["pon"]."</option>";		    
