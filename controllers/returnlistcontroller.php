...
require_once '../clases/paginator.class.php';
  ...
  $dbTable="viewcmareturn";
  $field="CONVERT(VARCHAR(10),fechanot,110) fechanot,numnotcre, nombres, medico,statnc,subtotal,descuento,totalnot,id,destatus,usuario";
  $concat="numnotcre,nombres,medico,destatus,totalnot" ;
  $key ="fechanot";
  ...
  if (  (isset($_POST['search']) && $_POST['search']!="")  && (isset($_POST['fecha']) && $_POST['fecha']!="")   ) {
     ...
     $filter=$_POST['search'];
     $fecha=$_POST['fecha'];
     $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where fechanot ='$fecha' and CONCAT( $concat ) like '%$filter%' " ;
     $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  fechanot ='$fecha'  and  CONCAT( $concat ) like '%$filter%'  ";
     ...
     if (is_numeric($filter) && $len>=6 ) {
        ...
        $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where CONCAT( $concat ) like '%$filter%' " ;
        $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  CONCAT( $concat ) like '%$filter%'  "; 
     }
