...
require_once '../clases/paginator.class.php';
  ...
  $dbTable="VIEW_MSSMfact_1 a";
  $field="CONVERT(VARCHAR(10),a.fechafac,110) fechafac,a.numfactu, a.nombres, a.Medico,a.statfact,a.subtotal,a.descuento,a.total,a.id,a.Status,a.numnotcre,a.Historia,a.usuario,a.codmedico,a.monto_abonado , (Select Case When sum(b.monto) IS NULL then 0 else  sum(b.monto)  End from Mpagos b Where b.numfactu=a.numfactu and b.tipo_doc='01' and id_centro=3 )  paid   ";

  if (  (isset($_POST['search']) && $_POST['search']!="")  && (isset($_POST['fecha']) && $_POST['fecha']!="")   ) {
         
     $filter=$_POST['search'];
     $fecha=$_POST['fecha'];
     $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where fechafac ='$fecha'  and CONCAT( numfactu,nombres,Historia,total,Medico) like '%$filter%' " ;
     $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  fechafac ='$fecha'  and   CONCAT( numfactu,nombres,Historia,total,Medico) like '%$filter%'  ";
     ...
     if (is_numeric($filter) && $len>=6 ) {
        ...
        $qryRows  = "SELECT COUNT(*) num_rows FROM ".$dbTable." Where CONCAT( numfactu,nombres,Historia,total,Medico) like '%$filter%' " ;
        $query    = "SELECT ".$field." FROM ".$dbTable." WHERE  CONCAT( numfactu,nombres,Historia,total,Medico) like '%$filter%'  "; 
     }
