...
function getConsultas(){
    $query="SELECT * from  MInventario  where activo=1  order by desitems";
...
function getAreThereDetails(){
 ...
 $query  = "SELECT  * FROM presupuesto_d Where numfactu ='$numfactu' " ;
...
if (isset($_GET['fac'])) {
 ...
 $query  = "SELECT  * FROM presupuesto_m Where numfactu ='$numfactu' " ;
 $res = mssqlConn::Listados($query);
 $results = json_decode($res, true);
 $statf=$results[0]['statfact'];

 $query  = "SELECT  * FROM view_presupuesto_1 Where numfactu ='$numfactu' " ;
 $res = mssqlConn::Listados($query);
 $results = json_decode($res, true);
 $len = sizeof($results);
 if($len>0){
