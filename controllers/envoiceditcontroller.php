...
function getConsultas(){
    $query="SELECT * from  MInventario where prod_serv in ('S','c') and activo = 1 order by ULTLOTE";
	...
	if($len>0){
		for ($i=0; $i <$len; $i++) { 
            ...
            if ($statf=='2' || $statf=='3' && $codperfil!=='01') {
            		$tabla .="<td ><select id=".$ids." name='serv[]' class='form-control service readonly' disabled  >";
            	}else{
            		$tabla .="<td ><select id=".$ids." name='serv[]' class='form-control service enterkey'   >";
            	}	
			...
			if ($statf=='2' || $statf=='3'  && $codperfil!=='01') {
				$tabla .="<td ><select id=".$idp." name='listaprecio[]'  class='form-control' disabled  >";
			}else{
				$tabla .="<td ><select id=".$idp." name='listaprecio[]'  class='form-control enterkey'   >";
			}
