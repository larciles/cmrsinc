...
var myAssociativeArrProd = [];
 $(document).ready(function(){  
  ...
  $('#xtitulo').html('<b>Consulta de presupuesto</b>');
...
var  tablesRows= document.getElementById("dynamic_field").rows.length;
  ...
  addButtom();
      ...
      $(".delete-row").click(function(){
             ...
             if (longArtr>1) {
                ...
                if (facturaNum!=="") {  
                       ...
                       $.ajax({  
                          url:"../../clases/presupuestosave.php",  
                          method:"POST",  
      ...
      $(document).on('click', '.btn_remove', function(){  
           ...
           if (facturaNum!=="") {
              ...
              $.ajax({                
                    url:"../../clases/updatepresupuesto.php",  
                    method:"POST",  
                    data: data ,  
                    success:function(data)  
                    {               
...
// 
  ...
  function setConsultas(id,data,type){
     ...
     var serv= getData("../../clases/getallproducts.php",data,type);
     items= jQuery.parseJSON(serv);
     var options;
     options+="<option value=''>Elija Producto</option>"; 
     myAssociativeArrProd = [];
     for (var j = 0; j < items.length; j++) { 
         options+="<option value='"+items[j].coditems+"'>"+items[j].desitems+" ("+parseInt(items[j].existencia)+")</option>"; 

         var newElement = {};
         newElement['id'] = items[j].coditems;
         newElement['taxable'] = items[j].aplicaIva;
         myAssociativeArrProd.push(newElement);
     }
     $("#"+id).html(options);
  }
...
$('#discamount').on('keydown', function(e) {
    if (e.which == 13) {
        e.preventDefault();
         do_DicountChanges()       
...
function do_DicountChanges(){
  ...
  function subTotal(coditems,codtipre,descuento,cantidad,id,precio){
    ...
    //\\
     ...
     url="../../clases/presupuestocalcularsubtotal.php";
     data = {
        coditems : coditems ,
        codtipre : codtipre,
        descuento : descuento,
    ...
    if (jQuery.type(items)!=='null') {
     ...
     if (DOMelemento.parent().parent().index()==0 && tax==0){
         var resptax=''; 
         url="../../clases/gettax.php";
         data = { };
         var itemTax='';
          resptax =  $.ajax({
                          type: "POST",
         ...
         itemTax= jQuery.parseJSON(resptax);
    ...
    if(jQuery.type( arraySubTotl.length )=='undefined'){
        ...
        for (var i = 0; i < arraySubTotl.length; i++) {
          if ( $("input[name='name[]']")[i].value!=="") {
              ...
              var cod_prod=$(".service")[i].value 

              var item = myAssociativeArrProd.find(item => item.id === cod_prod );
              ...
              if (taxable=="1") {   
                   ...
                   subtotaTaxable =subtotaTaxable+ (cantidad * precio);
               }else{
                   impuesto = 0;
                   subtot = (cantidad * precio);
                   subtotalNot = subtotalNot + (cantidad * precio);
              ...
              lineSubtotal=( subtotalNot+subtotaTaxable) ;// parseFloat( $("input[name='name[]']")[i].value ) ; //parseFloat(document.forms[0].elements["name[]"][i].value);
        ...
        calculosTotales(subtotaTaxable,tax,subtotalNot);
     }
...
function calculosTotales(subtotal,tax,notaxable){
  var montoDes=0;
  var total=0;
  var impuesto=0;
  ...
  if ($('#discprcntg').val()!=="" && $('#discprcntg')!==0) {
     ...
     total = total+notaxable+impuesto+shipping;
     ...
     $('#stmdct').val(subtotal-montoDes);
     $('#stmdct').html(subtotal-montoDes); 
  }else{   
     if (applytax) {
     ...
     impuesto = round(impuesto,2);
     subtotal = parseFloat(subtotal);
     total    = round((subtotal+notaxable+impuesto+shipping),2);     
...
//****************************************************************************************
  ...
  var numfactu=  $('#factura').val(); 
    ...
    url="../../clases/updatemedicopresupuesto.php";
  ...
  function displayTotales(numfactu){
    ...
    var res= getData("../../clases/getpresupuesto.php",data,'POST');
    ...
    $('#tlimpuesto').html(frmtax);  //VALOR MOSTRADO
    $('#frmtax').val(frmtax);   //VALOR POST

    if (parseInt(frmtax)==0) {
       $("#taxapply").trigger('click').prop('checked', false);
       $("#taxapply").trigger('click').prop('checked', false);       
    };
    //% DE IMPUSTO
    ...
    // SUBTOTAL MENOS EL DECUENTO 
    $('#stmdct').val(subtotal-porcenmonto);
    $('#stmdct').html(subtotal-porcenmonto);  
    ...
    $('#fecha').val(itemsf[0].fechafac2);
    setMedios(itemsf[0].medios);
    }
  }
//******************************************************
...
//*****************************************************
    ...
    if (prninvoice!='1' && autoposprn!='1' ) {
        window.open('../../clases/presupuestoprintpdf.php?numfactu='+_numfactu+'&times='+times+'&user='+user,'', '_blank');
    }else{
