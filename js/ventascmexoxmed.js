...
function getCitados(fechai,fechaf){
   var codperfil  =$('#codperfil').val();
    ...
    var items="";
    var ajxmed = $.post( "../../clases/repventascmexoxmed.php",{ fechai,fechaf}, function(data) { 
...
function fillTrTableCitados(data){
  ...
  for(var i = 0; i < data.length; i++){   
      ...
      html += '<tr><td align="center" >' + cont    +'</td>'+            
              ...
              '<td align="center" style='+estilo+' >'+ parseInt(data[i].cantidad)   +'</td>'+
              '<td align="center" style='+estilo+' >'  +'$ '+ parseFloat(data[i].neto).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')   +'</td>'+

              '</tr>';
              cantidadTotal=cantidadTotal+parseFloat(data[i].neto);
              unidadestotal=unidadestotal+parseInt(data[i].cantidad);  

  }

        html += '<tr><td align="center" >' + '-'    +'</td>'+            
              '<td align="center" >Total</td>'+           
              '<td align="center" style='+estilo+' >'+unidadestotal   +'</td>'+
              '<td align="center" style='+estilo+' >'  +'$ '+ cantidadTotal.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')   +'</td>'+
