 ...
 $(document).ready(function(){  
     $('#xtitulo').html('<b>Suero / Exos / Bloqueo</b>');
 	 var ii=0;
	...
	function setProductos(id){
		    var options;
         	...
         	//	options+="<option value='"+productos[j].coditems+"'>"+productos[j].desitems+"</option>"; 
         	         if ( productos[j].desitems.indexOf("CINTRON")>-1) {
          				  // options+="<option value='"+productos[j].coditems+"' style='background-color: aqua;font-size: 1.1em;'>"+productos[j].desitems+"</option>"; 

          	         	if (productos[j].coditems=='CTEX6LD' || productos[j].coditems=='IAPRP6LD1E' || productos[j].coditems=='IAPRP6LD1' ||  productos[j].coditems=='CMKCINTRON' || productos[j].coditems=='CMKCINTSEG' ) {
         	         		options+="<option value='"+productos[j].coditems+"' style='background-color: #ffab8a;font-size: 1.1em;'>"+productos[j].desitems+"</option>"; 
         	         	}else{
         	         		options+="<option value='"+productos[j].coditems+"' style='background-color: aqua;font-size: 1.1em;'>"+productos[j].desitems+"</option>"; 
         	         	}
             			...
             			if ( productos[j].desitems.indexOf("BLOQUEO")>-1) {
              				options+="<option value='"+productos[j].coditems+"' style='background-color:#FFA500;font-size: 1.1em;'>"+productos[j].desitems+"</option>"; 
             			}else{
              				options+="<option value='"+productos[j].coditems+"'>"+productos[j].desitems+"</option>"; 
                       }
         			}
	...
	function totales(){
        ...
        let total=parseFloat( subtotal-descuento_total).toFixed(2)
	...
	//-------------------------------------------------------------------------
	 $('#savei').click(function(){
	 	if (document.getElementById('desxmonto').value.trim()!="") {
	...
	function optionsChanged(id,pl,row_index){
				let params = 'id='+id+'&pl='+pl+'&price=y'; //+'&amp;pwd='+userPwd
		       	let url='../../handler/MsserviciosHandler.php';
		       	var api = new  XMLHttpRequest();
		       	api.open('POST',url,true);
		       	api.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		       	api.send(params);
	...
	//-------------------------------------------------------------------------
	 function getExclusivo(coditems){
	    ...
	    var url='../../handler/MsserviciosHandler.php';
