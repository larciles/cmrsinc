       ...
       if($('#manualpo').val()!=""){
           ...
           var confirmar = confirm('Seguro de agregar esta nueva Orden de Compra # '+pon+' ?');
            if (confirmar){
                ...
                var jqxhr = $.post( "../../controllers/Controller.php",{  action: 'ordenManual',pon : pon,type:'pr'  }, function() { 
                $('#manualpo').val('');
                  location.reload();  
                })
                .done(function() {
   ...
   $( '#btn-newpo' ).click(function() {
    ...
    var confirmar = confirm('Seguro de agregar esta nueva Orden de Compra # '+pon+' ?');
      if (confirmar){ 
        	...
        	var jqxhr = $.post( "../../controllers/Controller.php",{  action: 'pedidoAOrden', productos : obj ,pon : pon,type:'pr' }, function() { 
        	 $('#newpo').val('');
          	location.reload();  
      	  })
        	.done(function() {
...
function removeEditable(element) { 
  ...
  var jqxhr = $.post( "../../controllers/Controller.php",{ action: 'update_field_data', producto: userJson,type:'pr' }, function() {
  //alert( "success" );
...
// $('#CEMA011').addClass('table-success')
// $('#EXOS011').addClass('table-success')

document.getElementById('CEMA011').style="background-color:#0099C6;"
document.getElementById('EXOS011').style="background-color:#0099C6;"