...
getDataList();
  ...
  function getDataList(  page,  search,  fecha){
    if (page ==undefined) {
       page =1;
    };
     ...
     url="../../controllers/ajustelistacontroller.php";
     data = {
      limit  : 25,
      page   : page,
      search : search,
...
try{
  ...
  items= jQuery.parseJSON(resp);
    ...
    for (var i = 0; i < lenArr; i++) {
        ...
        var codajus  = items['data'][i]["codajus"];
        var usuario = items['data'][i]["usuario"];
        //trans=tranferidoYN(facclose);

        $('#tblListacompras').append('<tr id='+codajus+' >'+
         	' <td id='+codajus+' align="center">'+codajus+'</td>'+
...
$('#tblListacompras').on('click', 'tbody tr button', function(event) {
   ...
   if(elemt.attr('class').indexOf("devolver")>-1) {
       ...
       if(result.length==2){
          ...
          window.location.href = 'editajustes.php?idcompra='+numfactu;       
