...
function getCitados(fechai,fechaf){
   var codperfil  =$('#codperfil').val();
    ...
    var items="";
    var ajxmed = $.post( "../../clases/repunidadesvendidas.php",{ fechai,fechaf}, function(data) { 
...
function fillTrTableCitados(data){
  ...
  for(var i = 0; i < data.length; i++){   
      ...
      html += '<tr><td align="center" >' + cont    +'</td>'+              
              ...
              '<td align="left" >' + data[i].desitems     +'</td>'+
              '</tr>';
              cantidadTotal=cantidadTotal+parseInt(data[i].cantidad);

  }

  $("#tbodycant").html(html);
  $("#total").html('Total unidades vendidas '+cantidadTotal);
