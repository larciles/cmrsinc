  ...
  function getConsultasInvoices(  page,  search,  fecha){
    ...
    for (var i = 0; i < lenArr; i++) {
        ...
        //MEDICOS>
         ...
         if ( parseFloat( items['data'][i]["statfact"])=='1') {
            pagado="sin-pagar";
        ...
        // s
         ...
         let compare_total    = parseFloat(items['data'][i]["total"]).toFixed(2);
         if (compare_total!==paidtotal) {
            clasemiss = ' misspaid ';
         }else{
           clasemiss = '';
         }
        ...
        subtotal=parseFloat(subtotal).toFixed(2)
         ...
         $('#tblLista').append('<tr id='+numfactu+' class="'+clase+clasemiss+'">'+
          ...
          ' <td align="right">'+record+'</td>'+
         	...
         	' <td align="center"> <button type="button" name="devolver"  id=""  class="btn btn-warning devolver  '+pagado+' ">'+titleDev+'</button> </td>'+
...
$('#tblLista').on('click', 'tbody tr button', function(event) {
   ...
   if(elemt.attr('class').indexOf("devolver")>-1) {
       ...
       if(result.length==2){
          ...
          window.location.href = "msservicios-ret.php?return=true&numero="+numfactu;
       }
