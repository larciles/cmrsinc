...
function addButtom(){
      i++;  
          ...
          for (var ic = 0; ic < longarry; ic++) {
           ...
           setConsultas(idpro,{prod_serv:'', orderby:'desitems',notin},'POST');
...
// 
  ...
  function setConsultas(id,data,type){
   ...
   setConsultas('serv1',{prod_serv:'', orderby:'desitems'},'POST');
...
function calculosTotales(subtotal,tax,notaxable){
  ...
  if ($('#discprcntg').val()!=="" && $('#discprcntg')!==0) {
     ...
     $('#frmsubtotal').val(subtotal+notaxable);
     $('#frmtax').val(impuesto);
...
function loadIdsAsociados(items){    
  ...
  function loadMedicos(codMedOculto){
    ...
    function toDay(){
     ...
     $('#fecha').val(today)
     $('#fechaf').html(' '+today+'  <span id="invoicen">   Presupuesto #</span>') 
    }
