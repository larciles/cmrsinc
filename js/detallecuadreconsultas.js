  ...
  function getDataS(  page,  search,  fecha){
    ...
    if (page ==undefined) {
     ...
     if (fecha ==undefined) {
      ...
      url="../../clases/getsalesconsultasdet.php";
      data = {
      limit  : 500,
      page   : page,
      search : search,
...
function getTotalGrupos(fecha){    
    ...
    var jqxhr = $.post( "../../clases/getconsgrupototales.php",{fecha:fecha}, function(data) { 
...
function getPayMeth(fecha){
    url="../../clases/getmetodopagoconsultas.php";
      data = {
      fecha  : fecha
     }
    var items;
...
function pdfReporte(){
    ...
    if (fecha=='') {      
       fecha =$('#efechai').val();
    }else if($('#startdate').val()!=$('#efechai').val()){
      fecha =$('#efechai').val();
    };
    ...
    window.open('cuadre.php?fecha='+fecha+'&usuario='+usuario+'&idempresa='+'C'+'&start='+start+'&tiporeporte='+tiporeporte,'Cuadre de Consultas', '_blank');
