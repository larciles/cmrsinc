...
//--
 ...
 function getAllCashByUser(fecha, usuario){
  let tiporeporte = $('.tiporeporte').prop('checked');
  let items="";
  let ajxmed = $.post( "../../clases/getallcashbyuser.php",{ fecha,usuario,tiporeporte,id_centro:'3'}, function(data) { 
  let html = '';  
  let monto=0;
  let total=0;
...
function deleteCashUser(fecha,usuario){  
  let ajxmed = $.post( "../../clases/deletecashuser.php",{ fecha,usuario,id_centro:'3'}, function(data) { 
  items = jQuery.parseJSON(data);         
    if (typeof items!= 'undefined' && items!==null) {  
         window.location.reload(); 
    }
...
function getGrupo(fecha, usuario){
  ...
  var jqxhr = $.post( "../../clases/cuadrelasergrupo.php",{fecha, usuario,tiporeporte}, function(data) { 
  var items="";
  var users="";
  items= jQuery.parseJSON(data);
  var totalCash=0;
  ...
  if (items!==null) {
      ...
      if (totalCma==undefined) {
        var totalCma=totalCard;
      }else{
...
//-------------------------------------
  ...
  let tiporeporte = $('.tiporeporte').prop('checked');  
  var jqxhr = $.post( "../../clases/getcashlaser.php",{fecha:fecha,usuario:usuario,tiporeporte}, function(data) { 
  var items="";
  var users="";
  items= jQuery.parseJSON(data);
  var total=0;
...
function pdfReporte(){
   ...
   let start =$('#start').val();
   let tiporeporte = $('.tiporeporte').prop('checked');
   window.open('cuadre.php?fecha='+fecha+'&usuario='+usuario+'&idempresa='+'3'+'&start='+start+'&tiporeporte='+tiporeporte,'Cuadre de Consultas', '_blank');
...
$('#efechai').val(hoy);
  ...
  //-------------------------------------------------------------------------
  $('body').on('keydown', 'input,text,select,textarea', function(e) {
        var self = $(this)
