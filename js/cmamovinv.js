...
function getCitados(fechai,fechaf){
   var codperfil  =$('#codperfil').val();
   var items="";
    totalPage= getData("../../clases/paginacioncmahandler.php",{totalPage:'S',fechai,fechaf},'POST');
    Muestra_Paginacion(totalPage);
    let pagedat = getData("../../clases/paginacioncmahandler.php",{page:1,fechai,fechaf});
    items = jQuery.parseJSON(pagedat);         
    fillTrTableCitados(items);

    
    // var ajxmed = $.post( "../../clases/getcmamovinv.php",{ fechai,fechaf}, function(data) { 
...
function Muestra_Paginacion (totalPage) {
 ...
 $('.pagina').bootpag({
       ...
       maxVisible: 10
        ...
        let pagedat = getData("../../clases/paginacioncmahandler.php",{page:num,fechai,fechaf});
