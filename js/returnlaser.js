...
$('#medico').change(function(){
 ...
 $(document).ready(function(){  
  ...
  $('#xtitulo').html('<b>Devolucion Laser</b>');
      var i=1;  
      $('#add').click(function(){  
           i++;  
           var idpro="serv"+i;
           ...
           $('#dynamic_field').append('<tr id="'+i+'">'+
               ...
               ' <td><input type="text" name="subtotal[]"  readonly="readonly" placeholder="Subtotal" class="form-control name_list subtotal" /></td>'+
      ...
      $('#submit').click(function(){
          if ($('#idassoc').val()!=="" && $('#serv1').val()!=="" && $('#invoicen').val()=="") {        
               $.ajax({  
                    url:"../../clases/Invoicinglaserreturn.php",  
                    method:"POST",  
...
// 
 ...
 function changeItems(){
       $.ajax({  
                    url:"../../clases/changeitemdevolaser.php",  
                    method:"POST",  
                    data:$('#returnserv').serialize(),  
                    success:function(data)  
                    { 
  ...
  function setGeneralValues(element,index,xid){
      ...
      var codtipre  = $("[name='listaprecio[]']")[index].value ;// $('#tpre'+xid).val();        
...
function loadIdsAsociados(items){    
 var options=""; 
  ...
  $('#save').click(function(){
    ...
    returnPrinting(invoicen,2,'3',idusr);
