...
//  $("#bodyproducts").find("tr").remove();
  //  recipient = $('#idassoc').text();
  // var codclien  = $('#idassoc').val();
  recipient = $("#idassoc option:selected").text();
  var codclien  = $("#idassoc option:selected").val(); //$('#idassoc').val();
  var tipo="c"
  if (codclien=="") {

    var recno=$('#idpaciente').val()
    if (recno!="") {
       codclien=recno;
       tipo="r"
    }
  ...
  var data = getRecord(codclien,tipo);
  var items=[];
  var nFactura=0;

 if (recipient=="") {
    recipient=data[0].nombres;
 }
...
//----------------------
function getRecord(codclien,tipo){
  var items;
  var lretncitas;
  $.ajax({
            async:false,    
