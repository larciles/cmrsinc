$(function(){
	$('#xtitulo').html('<b>Pedidos Suero</b>');
       ...
       if($('#manualpo').val()!=""){
           ...
           var confirmar = confirm('Seguro de agregar esta nueva Orden de Compra # '+pon+' ?');
            if (confirmar){
                ...
                var jqxhr = $.post( "../../controllers/Controller.php",{  action: 'ordenManual',pon : pon,type:'st' }, function() { 
                $('#manualpo').val('');
                  location.reload();  
                })
                .done(function() {
   ...
   $( '#btn-newpo' ).click(function() {
    ...
    var confirmar = confirm('Seguro de agregar esta nueva Orden de Compra # '+pon+' ?');
      if (confirmar){ 
        	...
        	var jqxhr = $.post( "../../controllers/Controller.php",{  action: 'pedidoAOrden', productos : obj ,pon : pon,type:'st' }, function() { 
        	 $('#newpo').val('');
          	location.reload();  
      	  })
        	.done(function() {
...
function removeEditable(element) { 
  ...
  var jqxhr = $.post( "../../controllers/Controller.php",{ action: 'update_field_data', producto: userJson ,type:'st'}, function() {
  //alert( "success" );
