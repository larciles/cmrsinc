 ...
 $(document).ready(function(){  
     $('#xtitulo').html('<b>Devolución L A S E R</b>');
	...
	$('#idpaciente').on('keydown', function(e) {
		if (e.keyCode == 13) {
		        ...
		        url='../../handler/MsLaserRetHandler.php';
	...
	function totales(){
        ...
        if ($('.service').length>0) {
		        for (var i = 0; i < a_servicios.length; i++) {
		            ...
		            //LINEA DE SUBTOTAL
		            line_sub_des_tax=(line_subtotal-line_descuento) 
		    	...
		    	document.getElementById('discprcntg').value=parseFloat(0);
	...
	//-------------------------------------------------------------------------
	 $('#savei').click(function(e){
	 	e.preventDefault()

	 	if ($('#idassoc').val()==="" || $('#tlsubototal').val()==="" || parseFloat( $('#tlsubototal').val())===0) {

			Swal.fire('La Devolución está vacía');
	 		return;
	 	}
   ...
   //           document.getElementById('desxmonto').value=type_discnt;	 			
	...
	function optionsChanged(id,pl,row_index){
				let params = 'id='+id+'&pl='+pl+'&price=y'; //+'&amp;pwd='+userPwd
		       	let url='../../handler/MsLaserRetHandler.php';
		       	var api = new  XMLHttpRequest();
		       	api.open('POST',url,true);
		       	api.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		       	api.send(params);
	...
	//-------------------------------------------------------------------------
	 function getExclusivo(coditems){
	    ...
	    var url='../../handler/MsLaserRetHandler.php';
	...
	function invoicePrinting(){
		    ...
		    if (prninvoice!='1' && autoposprn!='1' ) {
		       ...
		       window.open('../../clases/printdevserviciospdf.php?numfactu='+_numfactu+'&times='+times+'&idempresa='+3+'&user='+user,'', '_blank'); 
