$(function(){
 ...
 $('#xtitulo').html('<b>Cuadre Consultas</b>');
...
function getAllCashByUser(fecha, usuario){
  let tiporeporte = $('.tiporeporte').prop('checked');
  let items="";
  let ajxmed = $.post( "../../clases/getallcashbyuser.php",{ fecha,usuario,tiporeporte,id_centro:'C'}, function(data) { 
  let html = '';  
  let monto=0;
  let total=0;
...
function deleteCashUser(fecha,usuario){  
  let ajxmed = $.post( "../../clases/deletecashuser.php",{ fecha,usuario,id_centro:'C'}, function(data) { 
  items = jQuery.parseJSON(data);         
    if (typeof items!= 'undefined' && items!==null) {  
         window.location.reload(); 
    }
...
function alert50(){
  ...
  //if (tiporeporte==false) {
    let dif=$('#dif').text();
    if (dif=="50" && ($('#start').val()=='0' || $('#start').val()=='' || $('#start').val()=='0.0') ){
       alert('Puede que falten los $50 del petty cash, asegurese por favor!') 
    }
    ;
  //}
}
...
function getGrupo(fecha, usuario){
  let tiporeporte = $('.tiporeporte').prop('checked');
  var jqxhr = $.post( "../../clases/cuadreconsultasgrupo.php",{fecha,tiporeporte,usuario}, function(data) { 
  var items="";
  var users="";
  items= jQuery.parseJSON(data);
  var totalCash=0;
...
function getCash(fecha, usuario){
  let tiporeporte = $('.tiporeporte').prop('checked');   
  if (usuario=='') {
     usuario = $('#loggedusr').val()
  };
  var jqxhr = $.post( "../../clases/getcashconsulta.php",{fecha:fecha,usuario:usuario,tiporeporte}, function(data) { 
  var items="";
  var users="";
  items= jQuery.parseJSON(data);
  var total=0;
