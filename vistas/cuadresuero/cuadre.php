...
$pdf->Cell(200 , 5, 'Sueroterapia'  ,0,1,'C');
...
for ($i=0; $i <sizeof($cheques) ; $i++) {  
    ...
    $pdf->Cell(52,4,$cheques[$i]['dmodopago'],0,0,'L','');
    if ((float) $cheques[$i]['monto']<0) {
         $pdf->Cell(38,4,'$'.number_format($cheques[$i]['monto']*-1, 2, ".", ","),0,1,'L','');
         $retchk=$retchk+( (float) $cheques[$i]['monto'] )*-1;
    }else{
         $pdf->Cell(38,4,'$'.number_format($cheques[$i]['monto'], 2, ".", ","),0,1,'L','');
    }
...
if ($bruta!=='') {
	...
	$pdf->Cell(30 , 5, '$'. number_format($bruta[0]['monto']+$retchk, 2, ".", ",")   ,0,1,'R');	
}
#------------------------------------------------- 
...
if ($devoluciones!=='') {
	$y2_axis = $pdf->GetY()  ;
    $pdf->SetXY(100,$y2_axis+1);  
	$factneta=($bruta[0]['monto']+$retchk + $devoluciones[0]['monto']);
...
if ($devoluciones!=='') {
	$y2_axis = $pdf->GetY()  ;
    $pdf->SetXY(100,$y2_axis+4);  
	$factneta=($bruta[0]['monto'] + $devoluciones[0]['monto']+$retchk);
	$facturas=($devoluciones[0]['facturas']+$bruta[0]['facturas']);
...
if ($idempresa=='S' ) {
    ...
    $query="SELECT sum(monto) monto, modopago  ,case when  tipo_doc='01' then modopago  when  tipo_doc='04' then 'RETURN CHECK' End  as dmodopago ,codforpa ,tipo_doc
        FROM VIEWpagosPRCMACST_1  
        where fechapago='$fecha' and 
        codforpa  in('2')  and 
        statfact=3   and 
        id_centro = '2' and 
        cod_subgrupo In('SUEROTERAPIA','CEL MADRE','BLOQUEO','INTRAVENOSO','TERAPIA LASER')  and usuario='$usuario' 
...
if ($idempresa=='1') {
    ...
    if ($tiporeporte=='false') {
        $query="SELECT sum(monto) monto, count(*) facturas from VIEWpagosPRCMACST_1 where fechafac='$fecha'  and statfact=3 and tipo_doc='01' and  cod_subgrupo In('SUEROTERAPIA','CEL MADRE','BLOQUEO','INTRAVENOSO','TERAPIA LASER') ";    
    }
}
$res = mssqlConn::Listados($query);
$obj= json_decode($res, true);
...
if ($idempresa=='1') {
    ...
    $query="SELECT sum( a.total) monto, count(*) facturas FROM VentasDiariasCMACSTWEB a INNER JOIN  MDocumentos b ON  a.doc = b.codtipodoc WHERE  a.fechafac = '$fecha' and a.doc='04' and statfact=3 and  cod_subgrupo  In('SUEROTERAPIA','CEL MADRE','BLOQUEO','INTRAVENOSO','TERAPIA LASER') and usuario='$usuario' ";        
    if ($tiporeporte=='false') {
        $query="SELECT sum( a.total) monto, count(*) facturas FROM VentasDiariasCMACSTWEB a INNER JOIN  MDocumentos b ON  a.doc = b.codtipodoc WHERE  a.fechafac = '$fecha' and a.doc='04' and statfact=3 and  cod_subgrupo  In('SUEROTERAPIA','CEL MADRE','BLOQUEO','INTRAVENOSO','TERAPIA LASER') ";        
    }
}
...
if ($idempresa=='1') {
    ...
    if ($tiporeporte=='false') {
        ...
        $query="SELECT  distinct a.numfactu, convert(varchar(10)
 ...
 , a.statfact
 , a.TotImpuesto
 , a.monto_flete FROM VentasDiariasCMACST1 a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc WHERE a.fechafac between  '$fecha' AND '$fecha' AND  a.statfact=3 and cod_subgrupo In('SUEROTERAPIA','CEL MADRE','BLOQUEO','INTRAVENOSO','TERAPIA LASER') And  a.total<0  /* and usuario='$usuario'  Order by a.numfactu desc */ "; 
    }
}
...
if ($idempresa=='1') {
    ...
    $query="SELECT sum(a.subtotal) subtotal , sum(a.descuento) descuento, sum( a.TotImpuesto) impuesto , sum(a.monto_flete) envio , sum( (a.total+a.monto_flete))  total   FROM  VentasDiariasCMACSTWEB a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc   WHERE a.fechafac between '$fecha' AND '$fecha'  AND  a.statfact=3 and cod_subgrupo In('SUEROTERAPIA','CEL MADRE','BLOQUEO','INTRAVENOSO','TERAPIA LASER')   And  a.total<0  and usuario='$usuario'  ";     
    if ($tiporeporte=='false') {
