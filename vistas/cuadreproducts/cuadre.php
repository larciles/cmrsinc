...
#-------------------------------------------------
$items =  getGrupo($fecha,$usuario,$idempresa,$tiporeporte);
$inicio=$start;
$pdf->SetXY(100,35);
...
$pdf->Cell(41 , 5, 'Total Medicina S.',0,0,'R');
$pdf->Cell(30 , 5, '$'.number_format($totalgeneral, 2, ".", ",") ,0,1,'R');
$pdf->SetFont('Arial');
#------------------------------------------------- 

$bruta = getFactBruta($fecha,$usuario,$idempresa,$tiporeporte);
...
#------------------------------------------------- 
$devoluciones = getDevoluciones($fecha,$usuario,$idempresa,$tiporeporte);
...
#========================================================================================================   
 ...
 $pdf->Cell(200 , 5, utf8_decode($sucursal[0]['Nombre'] .' - '.$sucursal[0]['sucursal'])  ,0,1,'C');
 $pdf->Cell(200 , 5, 'Reporte de Facturacion - Productos',0,1,'C');
 ...
 for ($i=0; $i <sizeof($ventas) ; $i++) { 
        ...
        $pdf->Cell($pdf->GetX()+3,4, $void.substr($ventas[$i]['nombres'], 0,25)  ,0,0,'L');
        ...
        $mpagos= getMetodoPagoByFactura($fecha,$usuario,$idempresa,$tiporeporte,$numfactu);
       // $pdf->Cell(0,4,$mpagos[$ventas[$i]['numfactu']],0,0,'L');
        $pdf->Cell(0,4,$mpagos,0,0,'L');
...
$pdf->SetFont('Arial','',9);
 ...
 for ($i=0; $i <sizeof($return) ; $i++) { 
         ...
         $pdf->Cell($pdf->GetX()+3,4, substr($return[$i]['nombres'], 0,25) ,0,0,'L');
         ...
         $numfactu=$return[$i]['numfactu'];
         $mpagos= getMetodoPagoByFactura($fecha,$usuario,$idempresa,$tiporeporte,$numfactu);
       // $pdf->Cell(0,4,$mpagos[$ventas[$i]['numfactu']],0,0,'L');
        //$pdf->Cell(0,4,$mpagos,0,0,'L');
        $pdf->Cell($pdf->GetX()+50,4,$mpagos,0,0,'L');
...
if ($idempresa=='1') {
    $query="SELECT convert(varchar(10), cast(a.fechafac as date),101)  fechapago ,a.numfactu, a.nombres, a.subtotal, a.descuento,  (a.total+a.monto_flete)  monto, a.statfact, a.TotImpuesto, a.monto_flete,a.Historia,a.initials  FROM  VentasDiarias a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc   WHERE a.fechafac between '$fecha' AND '$fecha'  And  a.total>=0 and a.usuario='$usuario'  Order by a.numfactu desc ";
    if ($tiporeporte=='false') {
        $query="SELECT convert(varchar(10), cast(a.fechafac as date),101)  fechapago ,a.numfactu, a.nombres, a.subtotal, a.descuento,  (a.total+a.monto_flete)  monto, a.statfact, a.TotImpuesto, a.monto_flete,a.Historia,a.initials  FROM  VentasDiarias a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc   WHERE a.fechafac between '$fecha' AND '$fecha'  And  a.total>=0  Order by a.numfactu desc ";
    }
}else if ($idempresa=='C' ) {
...
#--------------------------------------------------
function getMetodoPagoByFactura($fecha,$usuario,$idempresa='1',$tiporeporte,$numfactu){

            $pagos="";
            $split="";
            $query="SELECT d.numfactu, d.modopago From VIEWpagosPR d WHERE d.fechapago between '$fecha' AND '$fecha' and numfactu='$numfactu'  group by  d.numfactu,d.modopago Order By d.modopago";
            ...
            if ($lenObjt>0) {
                foreach($objt as $row){
                    $modopago=$modopago.substr($row['modopago'],0,8) ;
                    $modopago=$modopago.'/';
                }
                if (substr($modopago, -1)=="/") {
                    $modopago=substr($modopago,0, -1);
                }
                //$split=substr($modopago,0, -2);
            } 
...
if ($idempresa=='1') {
    $query="SELECT sum(a.subtotal) subtotal , sum(a.descuento) descuento, sum( a.TotImpuesto) impuesto , sum(a.monto_flete) envio , sum( (a.total))  total   FROM  VentasDiarias a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc   WHERE a.fechafac between '$fecha' AND '$fecha' AND  a.statfact=3  And  a.total<0 and a.usuario='$usuario' "; 
    if ($tiporeporte=='false') {
        $query="SELECT sum(a.subtotal) subtotal , sum(a.descuento) descuento, sum( a.TotImpuesto) impuesto , sum(a.monto_flete) envio , sum( (a.total))  total   FROM  VentasDiarias a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc   WHERE a.fechafac between '$fecha' AND '$fecha' AND  a.statfact=3  And  a.total<0  "; 
    }
}else if ($idempresa=='C' ) {
    $query="SELECT sum(a.subtotal) subtotal , sum(a.descuento) descuento, sum( a.TotImpuesto) impuesto , sum(a.monto_flete) envio , sum( (a.total))  total   FROM  VentasDiariasCMACST a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc   WHERE a.fechafac between '$fecha' AND '$fecha'  AND  a.statfact=3 and cod_subgrupo='CONSULTA'   And  a.total<0 "; 
}
...
if ($idempresa=='1') {
    ...
    if ($tiporeporte=='false') {
       $query="SELECT sum(a.subtotal) subtotal , sum(a.descuento) descuento, sum( a.TotImpuesto) impuesto , sum(a.monto_flete) envio , ( sum(a.subtotal)  - sum(a.descuento) + sum( a.TotImpuesto)  + sum(a.monto_flete)   )  total   FROM  VentasDiarias a INNER JOIN MDocumentos  b ON a.doc = b.codtipodoc   WHERE a.fechafac between '$fecha' AND '$fecha' AND  a.statfact=3   ";       
    }   
}else if ($idempresa=='C' ) {
