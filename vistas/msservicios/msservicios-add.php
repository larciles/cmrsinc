...
if (isset($_GET['edicion']) && $_GET['edicion']=="true") {
  ...
  $res = $cmamfacturacontroller->readUDF("Select *,REPLACE(CONVERT(CHAR(15), fechafac, 101), '', '-') AS fecha from cma_mfactura where cempresa='s' and statfact='1' and usuario='$user' and fechafac='$fecha' " );
}
$desxmonto="";
if ( sizeof($res)>0 ) {
    ...
    $fecha=$res[0]['fecha'];
    $media=$res[0]['medios'];
    $desxmonto=$res[0]['desxmonto'];

    $detf = $cmadfacturacontroller->readUDF("Select * from cma_dfactura where numfactu='$factura' " );

    $invetario=$cminventariocontroller->readUDF("select * from MInventario where Prod_serv in('s','c','f')  and cod_grupo='004' and cod_subgrupo IN ('SUEROTERAPIA','CEL MADRE','BLOQUEO','INTRAVENOSO','TERAPIA LASER','PACKAGE') and activo=1 order by orden, desitems " );
        ...
        <div class="tab-content">
          <div id="home" class="tab-pane fade in active">
                  ...
                  <form name="facturacion" id="facturacion" method="post">  
                    ...
                    <input  id="desxmonto" type="hidden" name="desxmonto" value="<?php echo($desxmonto) ?>">
                    ...
                    <div class="container-fluid">  
                      <div class="table-responsive yes">
                         <table class="table  table-hover table-condensed table-striped" id="dynamic_field">                                      
                              <thead class='thead' style='background-color:#259B9A;'>
                                  <tr>
                                       ...
                                       <?php
                                        for ($ip=0; $ip <sizeof($invetario) ; $ip++) { 
                                            ...
                                            <?php  
                                                  ...
                                                  $c=$invetario[$ip]['desitems'];      
                                                  $isCin=strrpos($invetario[$ip]['desitems'], "CINTRON") ;
                                                  if ($isCin>-1 ) {
                                                      $estilo="   style='background-color: aqua;font-size: 1.1em;'  ";                                               
                                                  }else{
                                                      $isRey=strrpos($invetario[$ip]['desitems'], "BLOQUEO") ;
                                                      if ($isRey>=0 && $isRey!=false ) {
                                                         $estilo="  style='background-color:#FFA500;font-size: 1.1em;'   ";
                                                      }
                                                  }
                                                ...
                                                <option value="<?php echo($invetario[$ip]['coditems'])?>"  <?php echo $estilo;?>  ><?php echo($invetario[$ip]['desitems'])?></option>
                        ...
                        <div class="table-responsive">
                          <table class="table">
                            <tbody>
                              ...
                              <tr>
                                <th>Total:</th>
                                <td><input type="text"  id="tltotal" name="tltotal"  value="<?php if(sizeof($res)>0 ) echo( number_format((float)  $res[0]['total']  ,  2, '.', '')  ) ; ?>" style="text-align:right;" class="form-control name_list" readonly /></td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
...
<script src="../../js/msservicios.js?v=20200201"></script>
<script src="../../js/msserviciospayments.js"></script>
<script src="../../js/listamsservicio.js?v=20190918"></script>
