...
$query="SELECT *  FROM Mmedicos  where activo=1 order by concat(nombre,apellido) ";
if (isset($_POST['all'])) {
    $query="SELECT *  FROM Mmedicos  order by concat(nombre,apellido) ";
}

$limit=25;
$response= $medcontroller->paginationUDF($limit,10,"pagination",$query);
...
?>
    ...
    for ($n=0; $n < count($result); $n++) { 
        ?>
            ...
            <tr>
                <td><?php echo $n+1;?> </td> 
                <td> <?php echo($result[$n]['nombre'] .' '.$result[$n]['apellido']) ; ?></td>
                <td> <?php echo($result[$n]['Codmedico']);  ?> </td>                
                <td>
