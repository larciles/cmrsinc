...
$query="SELECT   a.numfactu,  CONVERT(VARCHAR(10),a.fechafac,101) fechafac,  a.descuento, a.subtotal, a.total, a.statfact, a.usuario, a.codseguro, c.cantidad, c.precunit, b.direccionH, b.Historia, b.nombres, e.seguro, d.desitems, f.TipoDePago ,c.coditems, d.nombre_alterno
 "
         . "FROM  CMA_MFactura a     "
...
$pdf->Text(6, 10, 'CENTRO DE MEDICINA REGENERATIVA' ,1,0,'C',0);
$pdf->SetFont('Arial', '', 10);
$pdf->Text(3, 15, 'Av Dr Veve #51 Esq Calle Marti Bayamon PR' ,0,1,'R');
$pdf->Text(18,20, '00961 Phone 787-780-7575' ,0,1,'C');
$pdf->Text(27,25, "Invoice # ".$resP[0]['numfactu'] ,0,1,'C');
$pdf->Text(3, 30, "Patien or Resposible Part            ".$resP[0]['fechafac'] ,0,1,'C');
$pdf->SetFont('Arial', 'B', 10);
$pdf->Text(3, 35, $resP[0]['nombres'] ,0,1,'C');
$pdf->SetFont('Arial', '', 10);
...
for ($i=0; $i <$lenA ; $i++) { 
    ...
    if ($longitud>18) {
        ...
        $str = substr(ltrim(rtrim($resP[$i]['nombre_alterno'])),0,25);       
    ...
    $pdf->SetFont('Arial', '', 10);
    $pdf->Text(33,$pdf->GetY(), number_format((float)$resP[$i]['precunit'], 2, '.', '') ,0,0,'C');
    $pdf->Text(54,$pdf->GetY(), $resP[$i]['cantidad'] ,0,0,'C');
    $pdf->Text(65,$pdf->GetY(), number_format((float)($resP[$i]['cantidad']*$resP[$i]['precunit']), 2, '.', '') ,0,1,'L');
    $coorY+1;
    $pdf->SetY(($pdf->GetY()+5));
