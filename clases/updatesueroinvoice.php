...
if (isset($_POST['action']) && !empty($_POST['action'])){
    ...
    if ($action=='delete') {
        ...
        //$xx= $upInvoice->deleteRecord($coditems,$numfactu);   
...
class UpdateInvoice
{
	...
	public static function inventoryUpdate($coditems,$qty){
 	...
 	public static function deleteRecord($coditems,$numfactu){
 		$query="DELETE FROM CMA_DFactura WHERE coditems ='$coditems' AND numfactu='$numfactu'  ";
 		mssqlConn::insert($query);
 	}

    public static function insertUpdate(){
        $query="INSERT INTO CMA_DFactura   (numfactu,fechafac,fecreg,horareg,coditems,codseguro,codtipre,aplicaiva,aplicadcto,aplicacommed,aplicacomtec,costo,dosis,cant_sugerida,cantidad,precunit,descuento,monto_imp,workstation,ipaddress,usuario,Codmedico ) "
