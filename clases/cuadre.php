...
$pdf->Cell(200 , 5, 'CMR Consultas - $sucursal[0]['sucursal'] PR'  ,0,1,'C');
...
if ($devoluciones!=='') {
 ...
 $pdf->Cell(200 , 5, 'CMR Consultas - Bayamon PR'  ,0,1,'C');
...
// $pdf->Cell(200 , 5, 'Usuario : '.$usuario  ,0,1,'C');
 ...
 for ($i=0; $i <sizeof($ventas) ; $i++) { 
        ...
        $pdf->Cell($pdf->GetX()+50,4,$mpagos[$ventas[$i]['numfactu']],0,0,'L');
...
if ($idempresa=='1') {
     ...
     LEFT JOIN VIEWpagosPRCMA d ON a.numfactu = d.numfactu  
       WHERE
       a.fechafac ='$fecha' AND   a.cod_subgrupo = 'CONSULTA' and  a.statfact=3  and d.desTipoTargeta is  not null and usuario='$usuario' 
       GROUP BY  
       desTipoTargeta  
       Order by 
       desTipoTargeta ";
