...
$dbmsql = mssqlConn::getConnection();
 ...
 {  
  ...
  if ($_POST["discamount"] == "" || $_POST["discamount"] == "$0") {
   ...
   //$descuento   = ($_POST["discamount"] == "$0" ? "0" : $_POST["discamount"] );  //descuento
...
function getInvoiceNumber(){
  ...
  $query="UPDATE empresa SET UltimaFactura ='$invoiceNumber' where id_centro='1' ";
...
function saveMaster($codclien,$codmedico,$fechafac,$usuario,$subtotal,$descuento,$alicuota,$TotImpuesto,$frmshipping,$total,$iva,$seguro,$workstation,$ipaddress,$numfactu,$medio ){
 ...
 if ($numfactu!=="") {
  ...
  $query="SELECT *  from  MFactura  WHERE numfactu = '$invoiceNumber' ";
  $res = mssqlConn::Listados($query);
  $result = json_decode($res, true);
  $len = sizeof($result);
  if($len>0){
    //, usuario = '$usuario' 
     ...
     $query="UPDATE  MFactura  SET codclien ='$codclien',codmedico = '$codmedico',subtotal = '$subtotal' ,descuento = '$descuento',Alicuota='$alicuota',TotImpuesto = '$TotImpuesto',monto_flete = '$frmshipping',total = '$total',iva='$iva',codseguro= '$seguro',workstation='$workstation',ipaddress='$ipaddress',horareg='$hora',medios='$medio' Where numfactu='$numfactu' " ;
       mssqlConn::insert($query);
  }else{
     $recipe = ($codclien == "000" ? false : true); 
     $hora =  date("H:i:s", time()); 
     $query="INSERT INTO  MFactura (numfactu,codclien,codmedico,fechafac,usuario,subtotal,descuento,Alicuota,TotImpuesto,monto_flete,total,iva,codseguro,workstation,ipaddress,vencimiento,fecreg,horareg,recipe,cancelado,medios)"
...
$iskit=findKit($coditems,$cantidad,$invoiceNumber);
if ($iskit==0) {
...
$query="INSERT INTO DFactura   (numfactu,fechafac,fecreg,horareg,coditems,codseguro,codtipre,aplicaiva,aplicadcto,aplicacommed,aplicacomtec,costo,dosis,cant_sugerida,cantidad,precunit,descuento,monto_imp,workstation,ipaddress,usuario,Codmedico ) "
 ...
 function deleteRecords($invoiceNumber){
   $query="SELECT * from DFactura  WHERE numfactu = '$invoiceNumber' ";
   $res = mssqlConn::Listados($query);
   $result = json_decode($res, true);
   $len = sizeof($result);
   if($len>0){
      for ($i=0; $i <=  $len-1 ; $i++) { 
          ...
          $query="DELETE FROM DFactura WHERE numfactu = '$invoiceNumber' and coditems = '$coditems'";
          mssqlConn::insert($query);
      } 
   }
