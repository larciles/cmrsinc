...
$query = " SELECT "
        ...
        ." e.nombre, e.apellido, "   
        ." d.pago, "
        ." f.desitems, f.nombre_alterno , b.hCiudad,b.codpostal,b.ESTADO,b.Pais "
        ." FROM " 
        ." MFactura a "
...
$resP = json_decode($result, true);
 ...
 if(sizeof($resP)>0){
    ...
    if (strlen($resP[0]['ESTADO'])>0) {
       $query= "Select s.State from States s where s.Id='$state'";
       $result = mssqlConn::Listados($query);
       $estado_ar = json_decode($result, true);
        if (count($estado_ar)>0) {
             $estado=$estado_ar[0]['State'];
        }
    }
     }
    ///////////////////////////////////////
  ...
  if ($country!=160 && $country!=62 ) {
      
  
     if (strlen($resP[0]['Pais'])>0) {
          $query="Select p.PAIS from Pais p where p.Id='$country' ";
          $result = mssqlConn::Listados($query);
          $pais_ar = json_decode($result, true);
         if (count($pais_ar)>0) {
                $pais=$pais_ar[0]['PAIS']; 
       }
      }
    ...
    ////////////////////////////////////
     ...
     $ciudad=$resP[0]['hCiudad'];  
$dir=$ciudad.' '.$estado.' '.$pais.' '.$resP[0]['codpostal'];
}
...
//
$pdf->Text(3, $pdf->GetY()+5, $dir ,0,1,'C');
//
$pdf->SetY(($pdf->GetY()+5));
