...
if ($codconsulta=='07') {
 ...
 if ($nin>0) {
	 if ($codconsulta=='07') {
		...
		if($asistido ==0){
                    $query="DELETE FROM  mconsultasS  where codclien='$codclien' and asistido='0' and fecha_cita>'$fecha_cita'  and  CODITEMS ='$coditems' ";
		...
		//$query="UPDATE mconsultas   SET asistido = '$asistido', noasistido = '$noAsistido',regusuario = '$usuario' where codclien='$codclien' and fecha_cita='$fecha_cita' ";
                if($asistido ==0){
                    $query="DELETE FROM  mconsultas  where codclien='$codclien' and asistido='0' and fecha_cita>'$fecha_cita'";
                    $result = mssqlConn::insert($query);                     
                }    
