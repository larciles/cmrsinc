...
require('../controllers/PrescritolController.php');
...
//------------------------------------------------------------------
 ...
 function deleteRecords($invoiceNumber,$allYN,$coditems){
   ...
   $dfacturacontroller = new MSSDFactController();
   ...
   if ($allYN==true) {
    ...
    $mpagosController->del($invoiceNumber,'3','01');
    $idcardController->del($invoiceNumber,'3','01');
...
function save($factura){
    ...
    $mfacturaController = new MSSMFactController();
    $empresacontroller = new EmpresaController();
    $dfacturacontroller = new MSSDFactController();
    $prescritoLcontroller = new PrescritolController();

    $disponibleController = new DisplaserController();

    if (trim($factura)=="") {
        $factura=$empresacontroller->getLastNumber('factura','3',6);
    ...
    if($number > 0){
       for($i=0; $i<$number; $i++){
           if(trim($_POST["producto"][$i] !='')){
                ...
                if (!is_null($_POST["discprcntg"][$i] ) && !empty($_POST["discprcntg"][$i] ) && is_numeric($_POST["discprcntg"][$i]) ) {
        ...
        if ($_POST["desxmonto"]=='undefined') {
          $desxmonto=0;
                ...
                try {
                      ...
                      $respuesta =  $prescritoLcontroller->readWhere($array_edit);
                      ...
                      if ($longitud>0) {                         
                         ...
                         $array_edit = array(
                          ...
                          $prescritoLcontroller->update($array_edit);                          
                      }   


                  $set_data['codclien'] = $_POST["idassoc"];
                  $prescritoLcontroller->create($set_data);
                } catch (Exception $e) {
...
//------------------------------------------------------------------
 ...
 function saveCantDisponible($coditems,$qty,$codclien,$set_data ){
    ...
    if (count($kit)>0) {
      ...
      commitDisponible($codikit,$cantidad,$codclien,$set_data,$coditems);
    }
     
    }else{
     commitDisponible( $coditems,$qty,$codclien,$set_data,$coditems ); 
    }
    
}

function commitDisponible($coditems,$qty,$codclien,$set_data,$itemfac){
       $codclien=trim($codclien);
       $disponibleController = new DisplaserController();
       $mclientescontroller = new ClientesController();
       $recordp=$mclientescontroller->readUDF("Select Historia From MClientes Where codclien='$codclien'");
       $record="";
       if (count($recordp)>0) {
         $record=$recordp[0]['Historia'];
       }
          ...
          ,'cod_subgrupo'=>$set_data['cod_subgrupo']
...
union all
           SELECT numfactu,total,codclien,statfact, 'Suero-Amino' servicio from CMA_MFactura  where statfact='1' and  company='suero' and fechafac ='$fecha'  and codclien='$codclien' ";
